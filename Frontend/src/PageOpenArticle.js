...
class PageOpenArticle extends Component {
  ...
  updateContent() {
    ...
    if (AWSIP)
      requestedUrl =
        "http://ec2-52-14-208-196.us-east-2.compute.amazonaws.com:4000/getArticle?article_id=" +
        article_id +
        "&source=" +
        source;
    else
      requestedUrl =
        "http://127.0.0.1:4000/getArticle?article_id=" +
        article_id +
        "&source=" +
        source;
    ...
    fetch(requestedUrl)
      ...
      .then(
        (result) => {
          ...
          this.setState({
            newsCard: <BigNewsCard key={article_id} news={result[0]} />,
          });
        },
        (error) => {
          this.setState({
