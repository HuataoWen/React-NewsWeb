...
import MedianNewsCard from './components/MedianNewsCard';
...
class App extends React.Component {
        ...
        (result) => {
          ...
          setTimeout(
            function () {
              this.setState({ loading: false });
              this.setState({
                newsCard: this.state.news.map(news => {
                  return (
  ...
  removeFromBookmark(id, url, fromPage) {
    this.setState({
      bookmarkNews: this.state.bookmarkNews.filter(function (item) {
        return item.url !== url;
      })
    });
